...
#define MQTT_CLIENTID "ESP32_Garagentorsteuerung"  // MQTT-ClientID - muss eindeutig im Netzwerk sein
#define GT_MQTT_KEEPALIVE 90
#define GT_MQTT_SOCKETTIMEOUT 30
...
float timeTorAuf = DEFAULT_TIME_TOR_AUF;  // Gesamtzeit zum Öffnen des Garagentors [s]
float timeTorZu = DEFAULT_TIME_TOR_ZU;    // Gesamtzeit zum Schließen des Garagentors [s]
// Zeithysterese bei einer Auf-Auf-, Zu-Zu- oder schnellen Auf-Zu-Schaltung [s]
float timeHysterese = DEFAULT_TIME_HYSTERESE;
...
// Zustände
bool volatile torAuf = 0;  // Status der StateMaschine Zustand "TorAuf" - im Endanschlag - nächster Trigger -> Schließen
bool volatile torZu = 0;   // Status der StateMaschine Zustand "TorZu" - im Endanschlag - nächster Trigger -> Öffnen
bool volatile torUnDef = 0;  // Status der StateMaschine Zustand "TorUndefiniert" = irgendwas zwischen "Auf" und "Zu" &
                             // Fahrtrichtung bei Trigger ungekannt
...
bool volatile alertLowLevel_2 = 0;  // Alarmschwelle LowLevel_2 unterschritten
...
// Queue-Definition für Torsteuerung
...
void mqttCallback(char* topic, byte* message, unsigned int length) {
  ...
  // Tortrigger2 vor Tortrigger prüfen — startsWith("Tortrigger") würde sonst auch "Tortrigger2" matchen
  if ((tx_ac) && (str.startsWith("Tortrigger2"))) {
    mqttMessage = "Tortrigger2 ausgeführt";
    bool qData = true;
    BaseType_t rcQ;
    rcQ = xQueueSendToBack(qRelais, &qData, pdMS_TO_TICKS(100));
    if (debug > 2) Serial.println(mqttMessage);
    mqttPublishQueue(mqttTopicAC.c_str(), rcQ == pdPASS ? mqttMessage.c_str() : "Tortrigger2 FEHLER: Queue voll!",
                     false);
    tx_ac = 0;
  }
  if ((tx_ac) && (str.startsWith("Tortrigger"))) {
    ...
    rcQ = xQueueSendToBack(qRelais, &qData, pdMS_TO_TICKS(100));
    if (debug > 2) Serial.println(mqttMessage);
    mqttPublishQueue(mqttTopicAC.c_str(), rcQ == pdPASS ? mqttMessage.c_str() : "Tortrigger FEHLER: Queue voll!",
                     false);
    tx_ac = 0;
  }
  if ((tx_ac) && (str.startsWith("state="))) {
    ...
    s_queueData qData;
    str.remove(0, 6);
    qData.zustand = constrain(str.toInt(), 0, 100);
    qData.ticktime = xTaskGetTickCount();
    rcQ = xQueueSendToBack(torZustand, &qData, pdMS_TO_TICKS(100));
    if (rcQ == pdPASS) {
      mqttMessage = "state=";
      mqttMessage += String(qData.zustand);
      mqttMessage += " umgesetzt";
    } else {
      mqttMessage = "state= FEHLER: Queue voll!";
    }
    if (debug > 2) Serial.println(mqttMessage);
    mqttPublishQueue(mqttTopicAC.c_str(), mqttMessage.c_str(), false);
    tx_ac = 0;
  ...
  if ((tx_ac) && (str.startsWith("restart") || str.startsWith("reboot"))) {
    ...
    digitalWrite(RELAIS, HIGH);  // Relais sicherstellen: aus
...
void printStateMQTT() {
  ...
  String mqttJson;
  String mqttPayload;
  String wifiRSSI = (WiFi.status() == WL_CONNECTED) ? String(WiFi.RSSI()) : String("--");
  mqttTopic = MQTT_SERIAL_PUBLISH_STATE;
  mqttTopic += "JSON_0";
  mqttJson = "{\"lastError\":\"" + String(lastError) + "\"";
  mqttJson += ",\"WiFi_Signal_Strength\":\"" + wifiRSSI + "\"";
  mqttJson += ",\"WiFi_IP_Adress\":\"" + WiFi.localIP().toString() + "\"";
  mqttJson += ",\"WiFi_MAC_Adress\":\"" + WiFi.macAddress() + "\"";
  mqttJson += ",\"Torzustand_Prozent\":\"" + ((zustand >= 0) ? String(zustand) + " %" : String("unbekannt")) + "\"";
  mqttJson += ",\"Zustand\":\"";
  if (torAuf == 1)
    mqttJson += "Tor offen\"}";
  else if (torZu == 1)
    mqttJson += "Tor geschlossen\"}";
  else if (torUnDef == 1)
    mqttJson += "Torzustand unklar\"}";
  else
    mqttJson += "unbekannt\"}";
  if (debug > 2) Serial.println("MQTT_JSON: " + mqttJson);
  mqttPublishQueue(mqttTopic.c_str(), mqttJson.c_str(), false);
  ...
  mqttPublishQueue(mqttTopic.c_str(), mqttPayload.c_str(), false);
  if (debug > 2) Serial.print("MQTT lastError: ");
  if (debug > 2) Serial.println(mqttPayload);
  // WiFi Signalstärke
  mqttTopic = MQTT_SERIAL_PUBLISH_STATE;
  mqttTopic += "WiFi_Signal_Strength";
  mqttPayload = wifiRSSI;
  mqttPublishQueue(mqttTopic.c_str(), mqttPayload.c_str(), false);
  ...
  mqttPayload = (zustand >= 0) ? String(zustand) + " %" : "unbekannt";
  mqttPublishQueue(mqttTopic.c_str(), mqttPayload.c_str(), false);
...
static void getTempFromSensor(void* args) {
  ...
  for (;;) {  // Dauerschleife des Tasks
    ...
    tempCheck();     // Prüfung, ob Limits gerissen wurden
    rc = xSemaphoreGive(mutexTemp);
    assert(rc == pdPASS);
...
void triggerDoor() {
  ...
  // einschalten
  rc = xSemaphoreTake(mutexTor, portMAX_DELAY);
  assert(rc == pdPASS);
  digitalWrite(RELAIS, LOW);
  rc = xSemaphoreGive(mutexTor);
  assert(rc == pdPASS);
  // Wartezeit
  delay(TRIGGERTIME);
  // ausschalten
...
static void doorMotion(void* args) {
  ...
  for (;;) {  // Dauerschleife des Tasks
    // WDT-sicheres Warten auf Queue-Eintrag
    esp_task_wdt_reset();
    rc = xQueueReceive(qRelais, &motionTrigger, pdMS_TO_TICKS(5000));
    if (rc != pdPASS) continue;
    // Tor-Trigger wird ausgeführt
    triggerDoor();
    // weitere, anstehende Trigger werden verworfen
    rc = xQueueReset(qRelais);
    assert(rc == pdPASS);

    if (debug > 1) Serial.println("Stack frei doorMotion: " + String(uxTaskGetStackHighWaterMark(NULL) * 4) + " Bytes");

    // Task schlafen legen - check alle 500ms
    LEDblinkMsg();
    ticktime = xTaskGetTickCount();
    vTaskDelayUntil(&ticktime, 500);
  }
...
static void stateMaschine(void* args) {
  ...
  for (;;) {  // Dauerschleife des Tasks
    ...
    if (debug > 1)
      Serial.println("Stack frei stateMaschine: " + String(uxTaskGetStackHighWaterMark(NULL) * 4) + " Bytes");
...
static void window(void* args) {
  ...
  for (;;) {  // Dauerschleife des Tasks
    // WDT-sicheres Warten auf Queue-Eintrag
    esp_task_wdt_reset();
    rc = xQueueReceive(torZustand, &qDataTor, pdMS_TO_TICKS(5000));
    if (rc != pdPASS) continue;
    ...
    if (uxQueueMessagesWaiting(torZustand) == 0) {
      ...
      // Prüfe, ob Zustand undefiniert, oder Endanschläge gegeben
      rc = xSemaphoreTake(mutexStatus, portMAX_DELAY);
      assert(rc == pdPASS);
      posZu = positionZu;
      pos2 = position_2;
      ...
      if (posAuf == 1) {
        aktZustand = 100;  // aktueller Zustand = auf => 100%
      }
      if ((posAuf == 1) && (posZu == 1)) {
      ...
      if (aktZustand != qDataTor.zustand) {
        ...
        if (aktZustand < 0) {
          ...
          while (xQueueSendToBack(qRelais, &qData, pdMS_TO_TICKS(5000)) != pdPASS) {
            esp_task_wdt_reset();
          }
          // ticktime initialisieren
          ticktime = xTaskGetTickCount();
          ...
          while (running) {
            ...
            if (i * 500 > (int)(1.5 * waitTime)) {
              ...
              aktZustand = -1;
              running = false;
              // Tor anhalten — kein Endanschlag erreicht
              while (xQueueSendToBack(qRelais, &qData, pdMS_TO_TICKS(5000)) != pdPASS) {
                esp_task_wdt_reset();
              }
            }
        ...
        if (((posZu == 1) || (posAuf == 1)) && (aktZustand != qDataTor.zustand)) {
          // Endanschlag erreicht und Ziel verschieden vom aktuellen Stand => Fahrt auf Höhe möglich.
          ticktime = xTaskGetTickCount();
          // starte das Tor
          while (xQueueSendToBack(qRelais, &qData, pdMS_TO_TICKS(5000)) != pdPASS) {
            esp_task_wdt_reset();
          ...
          if (debug > 1) Serial.println(waitTime);
          if ((int)waitTime < TRIGGERTIME + 200) waitTime = (float)(TRIGGERTIME + 200);
          esp_task_wdt_reset();
          vTaskDelayUntil(&ticktime, (int)waitTime);
          // stoppe das Tor
          while (xQueueSendToBack(qRelais, &qData, pdMS_TO_TICKS(5000)) != pdPASS) {
            esp_task_wdt_reset();
          }
      ...
      if (debug > 1) Serial.println("Stack frei window: " + String(uxTaskGetStackHighWaterMark(NULL) * 4) + " Bytes");
    }
  }
}
...
void setup() {
  ...
  mqttClient.setKeepAlive(GT_MQTT_KEEPALIVE);
  mqttClient.setSocketTimeout(GT_MQTT_SOCKETTIMEOUT);
  mqttConnect();
  mqttTopic = MQTT_SERIAL_PUBLISH_BASIS + String("error");
  ...
  // Queue einrichten für Torzustand und Relaissteuerung
  ...
  zustand = -1;
  // CPU-ID für Task-Zuweisung ermitteln
  ...
  rc = xTaskCreatePinnedToCore(doorMotion,    // Taskroutine
                               "doorMotion",  // Taskname
                               3072,          // StackSize
                               nullptr,       // Argumente / Parameter
                               2,             // Priorität
  ...
  rc = xTaskCreatePinnedToCore(stateMaschine,    // Taskroutine
  ...
  rc = xTaskCreatePinnedToCore(window,    // Taskroutine
                               "window",  // Taskname
                               4096,      // StackSize
                               nullptr,   // Argumente / Parameter
                               2,         // Priorität
